...
class FrameManager:
    ...
    """
    def __init__(self, u, g, o, rootFolder, dataSubFolder, resultsSubFolder, fileRegEx="[0-9_]+[.]jpg"):
        self.rootFolder = rootFolder
        self.dataSubFolder = dataSubFolder
        self.resultsSubFolder = resultsSubFolder
        self.ugoSubPath = r"User %d\gesture %d\%s" % (u,g,o)
        self.fullPathToSource = r"%s\%s\%s" % (self.rootFolder, self.dataSubFolder, self.ugoSubPath)
        self.fullPathToResults = r"%s\%s\%s" % (self.rootFolder, self.resultsSubFolder, self.ugoSubPath)
        ...
        self.imageSetName = "U%dG%d%s" % (u,g,o)
        self.imageSetCount = 1
        self.imagesOnStack = max(len(self.imageNames), 0)
        self.currentImage = None
        self.currentImageName = None
        
        print(r"Number of images found in: " + self.ugoSubPath, " --> ", self.imagesOnStack)
    ...
    def setNextFrame(self):
        if self.imagesOnStack > 0:
            ...
            self.currentImage = CustomImage(self.currentImageName, join(self.fullPathToSource,self.currentImageName), "grayscale")
    ...
    def setCurrentImage(self, image):
        ...
        self.currentImage.distTransform = cv2.distanceTransform(self.currentImage.image.copy(),cv2.DIST_L2,3)
        self.currentImage.max_dT = self.currentImage.distTransform.max() 
...
#         self.mask.quickBinarize(250) #this was empty, but I put it at 250 to help movement mask region identification
        self.mask.quickBinarize() #this was empty, but I put it at 250 to help movement mask region identification
        
        #update properties of image
        self.mask.shape = np.shape(self.mask.image)
        self.mask.distTransform = cv2.distanceTransform(self.mask.image.copy(),cv2.DIST_L2,3)
        self.mask.max_dT = self.mask.distTransform.max()  
...
class CustomImage:
    ...
    def __init__(self, imageName, imagePath, color):
        ...
        self.distTransform = cv2.distanceTransform(self.image.copy(),cv2.DIST_L2,3)
        self.max_dT = self.distTransform.max() 
    ...
    def __readImageFromPath__(self):
        ...
        if exists(self.imagePath):
            if self.import_color.lower() == "color":
